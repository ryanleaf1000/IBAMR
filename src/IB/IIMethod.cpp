...
IIMethod::postprocessIntegrateData(double /*current_time*/, double /*new_time*/, int /*num_cycles*/)
{
    ...
    std::vector<std::vector<libMesh::PetscVector<double>*> > vec_collection_update = {
        d_U_new_vecs, d_X_new_vecs, d_U_n_new_vecs, d_U_t_new_vecs, d_F_half_vecs,d_smoothed_normal, d_half_X_vecs
    };
...
IIMethod::forwardEulerStep(const double current_time, const double new_time)
{
    ...
    for (unsigned int part = 0; part < d_num_parts; ++part)
    {
        ...
        d_X_half_vecs[part]->close();
        d_X_new_vecs[part]->close();
        //std::cout<<"I am called 1"<<std::endl;
    }
    return;
} // forwardEulerStep
...
IIMethod::trapezoidalStep(const double current_time, const double new_time)
{
    ...
    for (unsigned int part = 0; part < d_num_parts; ++part)
    {
        ...
        std::cout<<"I am called 3"<<std::endl;
        IBTK_CHKERRQ(ierr);
        d_X_new_vecs[part]->close();
        d_X_half_vecs[part]->close();
    }
...
IIMethod::computeLagrangianForce(const double data_time)
{
   ...
   // std::cout<<"lag"<<std::endl;
    ...
    for (unsigned part = 0; part < d_num_parts; ++part)
    {
        ...
        if(d_use_smoothed_normal || d_use_direct_coupling_smoothed_normal)
        {
            std::cout<<"smoothed number"<< i<<std::endl;
            for (auto el_it = el_begin; el_it != el_end; ++el_it)
            {
                auto elem = *el_it;
                const auto& Normal_dof_indices = smoothed_normal_dof_map_cache.dof_indices(elem);//F_dof_map_cache.dof_indices(elem);
                      ...
                      const double data_time)
{
    ...
    std::vector<std::vector<libMesh::PetscVector<double>*> > vec_collection_update = {
        d_X_IB_ghost_vecs, d_X_half_vecs, d_F_IB_ghost_vecs, d_F_half_vecs//,d_smoothed_normal_ghost,d_smoothed_normal
    };
    ...
    for (unsigned int part = 0; part < d_num_parts; ++part)
    {
        ...
        else
        {
            ...
            std::vector<std::string> vector_system_names{
                ...
                NORMAL_VELOCITY_SYSTEM_NAME, TANGENTIAL_VELOCITY_SYSTEM_NAME, FORCE_SYSTEM_NAME, SMOOTHED_NORMAL_SYSTEM_NAME, HALF_X_SYSTEM_NAME
            };
            std::vector<std::string> vector_variable_prefixes{ "X", "dX", "U", "U_n", "U_t", "F", "N","X_half" };
