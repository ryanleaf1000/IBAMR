...
static Timer* t_compute_fluid_traction;
static Timer* t_extrapolate_pressure_for_traction;
...
const std::string IIMethod::HALF_X_SYSTEM_NAME = "half X system";
...
void
IIMethod::registerTractionCalc(const unsigned int part)
{
    TBOX_ASSERT(part < d_num_parts);
...
IIMethod::preprocessIntegrateData(double current_time, double new_time, int /*num_cycles*/)
{
    ...
    d_half_X_systems.resize(d_num_parts);
    d_half_X_vecs.resize(d_num_parts);
    ...
    for (unsigned int part = 0; part < d_num_parts; ++part)
    {
        ...
        d_smoothed_normal[part]=dynamic_cast<PetscVector<double>*>(d_smoothed_normal_systems[part]->current_local_solution.get());
        ...
        d_half_X_systems[part] = &d_equation_systems[part]->get_system(HALF_X_SYSTEM_NAME);
        d_half_X_vecs[part] = dynamic_cast<PetscVector<double>*>(d_half_X_systems[part]->current_local_solution.get());
        ...
        *d_half_X_vecs[part] = *d_half_X_systems[part]->solution;
        if (d_use_pressure_jump_conditions)
        {
...
//    }
    for (unsigned part = 0; part < d_num_parts; ++part)
    {
        if (d_compute_fluid_traction[part])
        {
            vec_collection_update.push_back({d_WSS_in_half_vecs[part], d_WSS_out_half_vecs[part], d_TAU_in_half_vecs[part], d_TAU_out_half_vecs[part]});
        }
    }
    ...
    if (d_multistep_n_steps > 0)
    {
        ...
        //d_U_old_vecs
        if(d_use_stablized_tangent_velocity_update_structure){
            for (unsigned part = 0; part < d_num_parts; ++part)
            {
    ...
    for (unsigned part = 0; part < d_num_parts; ++part)
    {
        if (d_compute_fluid_traction[part])
        {
            ...
            calculateInterfacialFluidForces(p_data_idx, d_new_time, part); // TODO: Should this be half_time?
        }
    }
    ...
    for (unsigned part = 0; part < d_num_parts; ++part)
    {
        ...
        *d_half_X_systems[part]->solution = *d_X_half_vecs[part];
        *d_half_X_systems[part]->current_local_solution = *d_X_half_vecs[part];
        ...
        *d_smoothed_normal_systems[part]->current_local_solution = *d_smoothed_normal[part];
        ...
        //*d_half_X_systems[part]->solution = *d_half_X_vecs[part];
        //*d_half_X_systems[part]->current_local_solution = *d_half_X_vecs[part];
                              ...
                              const double data_time)
{
    ...
    for (unsigned int part = 0; part < d_num_parts; ++part)
    {
        ...
        NumericVector<double>* WSS_in_vec = d_compute_fluid_traction[part] ? d_WSS_in_half_vecs[part] : nullptr;
        NumericVector<double>* WSS_out_vec = d_compute_fluid_traction[part] ? d_WSS_out_half_vecs[part] : nullptr;
        ...
        std::vector<std::vector<unsigned int> > smoothed_normal_dof_indices(NDIM);
        ...
        const std::vector<double>& JxW_smoothed_normal = fe_smoothed_normal->get_JxW();
        const std::vector<std::vector<double> >& phi_smoothed_normal = fe_smoothed_normal->get_phi();
        ...
        std::unique_ptr<FEBase> fe_F = FEBase::build(dim, F_fe_type);
        const std::vector<double>& JxW_F = fe_F->get_JxW();
        const std::vector<std::vector<double> >& phi_F = fe_F->get_phi();
        ...
        std::unique_ptr<NumericVector<double> >  X_corrected_rhs_vec=X_vec->zero_clone();
        std::unique_ptr<NumericVector<double> > smoothed_normal_rhs_vec = smoothed_normal_vec->zero_clone();
        std::vector<DenseVector<double> > U_rhs_e(NDIM);
        std::vector<DenseVector<double> > X_corrected_rhs_e(NDIM);
        ...
        std::vector<DenseVector<double>> smoothed_normal_rhs_e(NDIM);
        std::unique_ptr<NumericVector<double> > WSS_out_rhs_vec =
            (d_compute_fluid_traction[part] ? WSS_out_vec->zero_clone() : std::unique_ptr<NumericVector<double> >());        DenseVector<double> WSS_out_rhs_e[NDIM];

        std::unique_ptr<NumericVector<double> > WSS_in_rhs_vec =
            (d_compute_fluid_traction[part] ? WSS_in_vec->zero_clone() : std::unique_ptr<NumericVector<double> >());
        ...
        //            {
        //                auto elem = *el_it;
        //                const auto& Normal_dof_indices = smoothed_normal_dof_map_cache.dof_indices(elem);//F_dof_map_cache.dof_indices(elem);
        //                const auto& X_dof_indices = X_dof_map_cache.dof_indices(elem);
        //
        //                for (unsigned int d = 0; d < NDIM; ++d)
        //                {
        //                    smoothed_normal_rhs_e[d].resize(static_cast<int>(Normal_dof_indices[d].size()));
        //                }
        //                fe_X->reinit(elem);
        //
        ...
        //                {
        //                    interpolate(x_dub, qp, x_node, phi_X);
        ...
        //                    {
        //                        Normal_qp = n* phi_X[k][qp] * JxW[qp];//n * phi_X[k][qp] * JxW[qp];
        //                        for (unsigned int i = 0; i < NDIM; ++i)
        //                        {
        //                            smoothed_normal_rhs_e[i](k) += Normal_qp(i);
        //                        }
        //                    }
        //                }
        ...
        //                    smoothed_normal_dof_map->constrain_element_vector(smoothed_normal_rhs_e[i], dof_id_scratch);// F_dof_map.constrain_element_vector(Normal_rhs_e[i], dof_id_scratch);
        //                    smoothed_normal_rhs_vec->add_vector(smoothed_normal_rhs_e[i], dof_id_scratch);
        //                }
        //            }
        //            //SAMRAI_MPI::sumReduction(&F_integral(0), NDIM);
        //            smoothed_normal_rhs_vec->close();
        //            d_fe_data_managers[part]->computeSmoothedL2Projection(
        //                    *smoothed_normal_vec, *smoothed_normal_rhs_vec, SMOOTHED_NORMAL_SYSTEM_NAME, d_normal_stabilization_eps);
        //            smoothed_normal_vec->close();
        //        }
        ...
        if(false){
            for (PatchLevel<NDIM>::Iterator p(level); p; p++, ++local_patch_num) {
                ...
                qrule.reset();
                for (unsigned int e_idx = 0; e_idx < num_active_patch_elems; ++e_idx) {
                    //std::cout<<"I am iterative at layer:"<<layer<<std::endl;
                    ++layer;
                    Elem *const elem = patch_elems[e_idx];
                    for (unsigned int d = 0; d < NDIM; ++d) {
                        smoothed_normal_dof_map_cache.dof_indices(elem, smoothed_normal_dof_indices[d], d);
                        smoothed_normal_rhs_e[d].resize(static_cast<int>(smoothed_normal_dof_indices[d].size()));
                        X_dof_map_cache.dof_indices(elem, X_dof_indices[d], d);
                    }
                    get_values_for_interpolation(x_node, *X_ghost_vec, X_dof_indices);
                    const bool qrule_changed =
                        FEDataManager::updateInterpQuadratureRule(qrule, d_default_interp_spec, elem, x_node,
                                                                  patch_dx_min);
                    if (qrule_changed) fe_X->attach_quadrature_rule(qrule.get());

                    fe_X->reinit(elem);
                    const unsigned int n_qpoints = qrule->n_points();
                    const size_t n_basis = smoothed_normal_dof_indices[0].size();
                    for (unsigned int qp = 0; qp < n_qpoints; ++qp) {
                        for (unsigned int k = 0; k < NDIM - 1; ++k) {
                            interpolate(dx_dxi[k], qp, x_node, *dphi_dxi[k]);
                        }
                        ...
                        for (unsigned int k = 0; k < n_basis; ++k) {
                            ...
                            for (unsigned int d = 0; d < NDIM; ++d) {
                                smoothed_normal_rhs_e[d](k) += n(d) * p_JxW;
                            }
                        }

                    }
                    for (unsigned int d = 0; d < NDIM; ++d) {
                        smoothed_normal_dof_map->constrain_element_vector(smoothed_normal_rhs_e[d],
                                                                          smoothed_normal_dof_indices[d]);
                        smoothed_normal_rhs_vec->add_vector(smoothed_normal_rhs_e[d], smoothed_normal_dof_indices[d]);
                    }
                }
                smoothed_normal_rhs_vec->close();
                d_fe_data_managers[part]->computeSmoothedL2Projection(
                    *smoothed_normal_vec, *smoothed_normal_rhs_vec, SMOOTHED_NORMAL_SYSTEM_NAME, 0);
                smoothed_normal_vec->close();
            }
        }
        /////////////////////////////
        copy_and_synch(*smoothed_normal_vec, *smoothed_normal_ghost_vec);
        smoothed_normal_ghost_vec->close();
        ...
        for (PatchLevel<NDIM>::Iterator p(level); p; p++, ++local_patch_num)
        {
            ...
            for (unsigned int e_idx = 0; e_idx < num_active_patch_elems; ++e_idx)
            {
                ...
                if(data_time<d_Q2_correction_end_time&&d_Q2_mesh_correction){
                    ...
                    int tri_nodes_num=elem->n_nodes();
                    for (int tri_nodes=0; tri_nodes<tri_nodes_num;++tri_nodes){
                        libMesh::Point node_cache=elem->point(tri_nodes);
                        double r_mesh= sqrt(node_cache(0)*node_cache(0)+node_cache(1)*node_cache(1));
                        elem->point(tri_nodes)=libMesh::Point(node_cache(0)*radius/r_mesh,node_cache(1)*radius/r_mesh,node_cache(2));
                        std::cout<<(node_cache-elem->point(tri_nodes))<<std::endl;
                    }
                }
            ...
            F_trial_qp.resize(NDIM * n_qpoints_patch);
            for (unsigned int axis = 0; axis < NDIM; ++axis)
            {
            ...
            for (unsigned int e_idx = 0; e_idx < num_active_patch_elems; ++e_idx)
            {
                ...
                get_values_for_interpolation(x_node, *X_ghost_vec, X_dof_indices);
                get_values_for_interpolation(smoothed_normal_node, *smoothed_normal_ghost_vec, smoothed_normal_dof_indices);
                get_values_for_interpolation(F_node, *F_ghost_vec, F_dof_indices);


                //                if(d_use_smoothed_normal){
                //                    get_values_for_interpolation(Normal_node, *Normal_vec, U_dof_indices);
                //                }
                if (d_use_velocity_jump_conditions)
                {
                    for (unsigned int axis = 0; axis < NDIM; ++axis)
                ...
                // qp.
                std::vector<libMesh::Point> qp_points = qrule->get_points();
                for(unsigned int k = 0; k < n_nodes; ++k)
                {
                    for(unsigned int qp = 0; qp < n_qpoints; ++qp)
                    {
                        VectorValue<double> x_nodal;
                        const double& p = phi_X[k][qp];
                        for (unsigned int d = 0; d < NDIM; ++d)
                        {
                            x_qp[NDIM * (qp_offset + qp) + d] += x_node[k][d] * p;
                        }
                        if(d_use_direct_coupling_smoothed_normal || d_use_smoothed_normal_for_traction){ //(d_smoothed)
                            const double& p_smoothed_normal = phi_X[k][qp];
                            for (unsigned int d = 0; d < NDIM; ++d)
                            {
                                n_qp[NDIM * (qp_offset + qp) + d] += smoothed_normal_node[k][d] *p_smoothed_normal;
                            }
                        }
                        if(d_F_trial){
                            const double& p_smoothed_normal = phi_X[k][qp];
                            for (unsigned int d = 0; d < NDIM; ++d)
                            {
                                F_trial_qp[NDIM * (qp_offset + qp) + d] += F_node[k][d] * p_smoothed_normal;
                            }
                        }
                        if (d_use_velocity_jump_conditions)
                        {
                ...
                for (unsigned int qp = 0; qp < n_qpoints; ++qp)
                {
                    ...
                    for (unsigned int d = 0; d < NDIM; ++d)
                    {
                        ...
                        if(d_use_direct_coupling_smoothed_normal || d_use_smoothed_normal_for_traction){
                            double n_magnitude =0;
                            if(NDIM==2){ n_magnitude=sqrt(n_qp[NDIM * (qp_offset + qp)]*n_qp[NDIM * (qp_offset + qp)]+n_qp[NDIM * (qp_offset + qp)+1]*n_qp[NDIM * (qp_offset + qp)+1]);}
                            if(NDIM==3){ n_magnitude=sqrt(n_qp[NDIM * (qp_offset + qp)]*n_qp[NDIM * (qp_offset + qp)]+n_qp[NDIM * (qp_offset + qp)+1]*n_qp[NDIM * (qp_offset + qp)+1]+n_qp[NDIM * (qp_offset + qp)+2]*n_qp[NDIM * (qp_offset + qp)+2]);}
                            n_qp[NDIM * (qp_offset + qp) + d]/=n_magnitude;
                            n(d)= n_qp[NDIM * (qp_offset + qp) + d];
                        }
                        else{
                    ...
                    //if((n(0)-n_qp[NDIM * (qp_offset + qp)])*(n(0)-n_qp[NDIM * (qp_offset + qp)])+(n(1)-n_qp[NDIM * (qp_offset + qp)+1])*(n(1)-n_qp[NDIM * (qp_offset + qp)+1])+(n(2)-n_qp[NDIM * (qp_offset + qp)+2])*(n(2)-n_qp[NDIM * (qp_offset + qp)+2])>0.1){
                    //std::cout<<"x="<<x_qp[NDIM * (qp_offset + qp)]<<","<<x_qp[NDIM * (qp_offset + qp)+1]<<","<<x_qp[NDIM * (qp_offset + qp)+2]<<")"<<std::endl;
                    //std::cout<<"FE n=("<<n_qp[NDIM * (qp_offset + qp)]<<","<<n_qp[NDIM * (qp_offset + qp)+1]<<","<<n_qp[NDIM * (qp_offset + qp)+2]<<")"<<std::endl;
                    //std::cout<<"FE n=("<<F_trial_qp[NDIM * (qp_offset + qp)]<<","<<F_trial_qp[NDIM * (qp_offset + qp)+1]<<","<<F_trial_qp[NDIM * (qp_offset + qp)+2]<<")"<<std::endl;
                    // }
                }
                qp_offset += n_qpoints;
            }
            ...
            else if (u_sc_data && d_use_velocity_jump_conditions)
            {
                ...
                for (unsigned int axis = 0; axis < NDIM; ++axis)
                {
                    ...
                    for (unsigned int k = 0; k < local_indices.size(); ++k)
                    {
                        ...
                        VectorValue<double> norm_vec, du_jump, coeff_vec, wrc,F_trial;
                        // Loop over indices to calculate the interp coefficients (Lower=0, Upper=1)

                        for (int d = 0; d < NDIM; ++d) norm_vec(d) = n_qp[s * NDIM + d];
                        if(d_F_trial){for (int d = 0; d < NDIM; ++d) F_trial(d) = F_trial_qp[s * NDIM + d];}
                        ...
                        for (int d = 0; d < NDIM; ++d)
                        {
                            for (BoxIterator<NDIM> b(stencil_box); b; b++)
                            {
                                ...
                                if (d_use_direct_coupling)
                                {
                                    ...
                                    if(d_F_trial){
                                        for (int j = 0; j < NDIM; ++j){
                                            du_jump_sub(j) = -(F_trial(d) - F_trial * norm_vec * norm_vec(d)) * norm_vec(j);}
                                    }
                                    else{
                                        for (int j = 0; j < NDIM; ++j){
                                            du_jump_sub(j) = (du_jump(d) - du_jump * norm_vec * norm_vec(d)) * norm_vec(j);}
                                    }
                                    ...
                                    // std::cout<<k<<d<<ic[0]<<ic[1]<<":forces:"<<du_jump<<std::endl;
                                    //  std::cout<<k<<d<<ic[0]<<ic[1]<<":normal vector:"<<norm_vec<<std::endl;
                                ...
                                if (d_use_direct_coupling)
                                {
                                    ...
                                    Ujump[ic[0]][ic[1]][ic[2]][d] = dx[0] * w[0][ic[0] - ic_lower[0]] *
                                                                    ...
                                                                    (wrc *  du_jump_sub);
                                    if((x[0]-norm_vec(0))*(x[0]-norm_vec(0))+(x[1]-norm_vec(1))*(x[1]-norm_vec(1))>0.00001){
                                        // std::cout<<"location:"<<x[0]<<","<<x[1]<<","<<x[2]<<std::endl;
                                        //std::cout<<"norm_vec:"<<norm_vec<<std::endl;
                                        //std::cout<<"du_jump:"<<du_jump<<std::endl;
                                    }
                                }
                                else{
                                    Ujump[ic[0]][ic[1]][ic[2]][d] = dx[0] * w[0][ic[0] - ic_lower[0]] *
                                                                    ...
                                                                    (wrc * du_jump);
                                    //                                   std::cout<<k<<d<<ic[0]<<ic[1]<<":normal vector:"<<norm_vec<<std::endl;
                                    //                                    std::cout<<k<<d<<ic[0]<<ic[1]<<":split forces:"<<F_trial<<std::endl;
                                }
#endif
                            }
                        }
                        ...
                        for (BoxIterator<NDIM> b(stencil_box); b; b++)
                        {
                            ...
                            if (d_use_velocity_jump_conditions && d_use_velocity_correction)//&& !MathUtilities<double>::equalEps(data_time, d_new_time))
                            {
...
//                            }
            ...
            for (unsigned int e_idx = 0; e_idx < num_active_patch_elems; ++e_idx)
            {
                ...
                for (unsigned int d = 0; d < NDIM; ++d)
                {
                    ...
                    if(data_time<d_Q2_correction_end_time&&d_Q2_mesh_correction){
                        X_corrected_rhs_e[d].resize(static_cast<int>(X_dof_indices[d].size()));
                    }
                    X_dof_map_cache.dof_indices(elem, X_dof_indices[d], d);
                    if (d_compute_fluid_traction[part])
                    {
...
//
                    ...
                    for (unsigned int d = 0; d < NDIM; ++d)
                    {
                        ...
                        if(data_time<d_Q2_correction_end_time&&d_Q2_mesh_correction)
                        {
                            ...
                            double r_mesh = sqrt(x_qp[NDIM * (qp_offset + qp)] * x_qp[NDIM * (qp_offset + qp)] +
                                                 x_qp[NDIM * (qp_offset + qp) + 1] * x_qp[NDIM * (qp_offset + qp) + 1]);
                            if (d == 1 || d == 0)
                            {
                                X_corrected(d) = x_qp[NDIM * (qp_offset + qp) + d] * radius / r_mesh;
                            }
                            else
                            {
                                X_corrected(d) = x_qp[NDIM * (qp_offset + qp) + d];
                            }
                        }
                        if (d_compute_fluid_traction[part])
                        {
                    ...
                    if(d_output_training_data){
                        if (NDIM == 2) {
                            error_output << x_qp[NDIM * (qp_offset + qp)] << " "<< x_qp[NDIM * (qp_offset + qp)+1] << " "<< (U * n) << "\n";
                    ...
                    if(data_time<d_Q2_correction_end_time&&d_Q2_mesh_correction)
                    {
                        for (unsigned int k = 0; k < n_basis; ++k)
                        {
                            ...
                            for (unsigned int d = 0; d < NDIM; ++d)
                            {
                                X_corrected_rhs_e[d](k) += X_corrected(d) * p_JxW;
                            }
                        }
                    }
                    if (d_compute_fluid_traction[part])
                ...
                for (unsigned int d = 0; d < NDIM; ++d)
                {
                    ...
                    if(data_time<d_Q2_correction_end_time&&d_Q2_mesh_correction)
                    { X_dof_map.constrain_element_vector(X_corrected_rhs_e[d], X_dof_indices[d]);
                        X_corrected_rhs_vec->add_vector(X_corrected_rhs_e[d], X_dof_indices[d]);}
        ...
        if (d_compute_fluid_traction[part])
        {
            ...
            d_fe_data_managers[part]->computeSmoothedL2Projection(
                *WSS_in_vec, *WSS_in_rhs_vec, WSS_IN_SYSTEM_NAME, d_wss_stabilization_eps);
            WSS_in_vec->close();

            WSS_out_rhs_vec->close();
            d_fe_data_managers[part]->computeSmoothedL2Projection(
                *WSS_out_vec, *WSS_out_rhs_vec, WSS_OUT_SYSTEM_NAME, d_wss_stabilization_eps);
            WSS_out_vec->close();
        }
...
{
    IBAMR_TIMER_START(t_compute_fluid_traction);
    ...
    NumericVector<double>* Smoothed_Normal_vec = d_smoothed_normal[part];
    std::unique_ptr<NumericVector<double> > Smoothed_Normal_rhs_vec = Smoothed_Normal_vec->zero_clone();
    std::array<DenseVector<double>, NDIM> Smoothed_Normal_rhs_e;
    VectorValue<double> Smoothed_Normal_qp;

    VectorValue<double> Smoothed_normal;
    const std::vector<std::vector<Elem*> >& active_patch_element_map =
        d_fe_data_managers[part]->getActivePatchElementMap();

    boost::multi_array<double, 2> x_node, X_node, WSS_in_node, WSS_out_node, n_qp_node,Smoothed_Normal_node;
    ...
    for (PatchLevel<NDIM>::Iterator p(level); p; p++, ++local_patch_num)
    {
        ...
        if(d_use_smoothed_normal_for_traction)
        {
            std::vector<libMesh::dof_id_type> dof_id_scratch;
            qrule.reset();
            unsigned int qp_offset = 0;
            for (auto el_it = 0; el_it != num_active_patch_elems; ++el_it)
            {

                Elem* const elem = patch_elems[el_it];
                const auto& X_dof_indices = X_dof_map_cache.dof_indices(elem);
                const auto& Normal_dof_indices = smoothed_normal_dof_map_cache.dof_indices(elem);//F_dof_map_cache.dof_indices(elem);
                //                for (unsigned int d = 0; d < NDIM; ++d)
                //                {
                ...
                const size_t n_basis = phi_X.size();
                const size_t n_basis2 = phi_X.size();
                for (unsigned int qp = 0; qp < n_qpoints; ++qp)
                {
                    for (unsigned int k = 0; k < NDIM - 1; ++k)
                    {
                    ...
                    for (unsigned int k = 0; k < n_basis; ++k)
                    {
                        Smoothed_Normal_qp = n * phi_X[k][qp] * JxW[qp];//n * phi_X[k][qp] * JxW[qp];
                        for (unsigned int i = 0; i < NDIM; ++i)
                        {
                            Smoothed_Normal_rhs_e[i](k) += Smoothed_Normal_qp(i);
                        }
                    }
                }
                // Apply constraints (e.g., enforce periodic boundary conditions)
        ...
        for (unsigned int e_idx = 0; e_idx < num_active_patch_elems; ++e_idx)
        {
            ...
            fe_P->reinit(elem);
            if(d_use_smoothed_normal_for_traction){
                get_values_for_interpolation(Smoothed_Normal_node, *Smoothed_Normal_vec,smoothed_normal_dof_indices);
            }
    ...
    d_fe_data_managers[part]->computeSmoothedL2Projection(
        *TAU_in_vec, *TAU_in_rhs_vec, TAU_IN_SYSTEM_NAME, d_tau_stabilization_eps);
    TAU_in_vec->close();

    d_fe_data_managers[part]->computeSmoothedL2Projection(
        *TAU_out_vec, *TAU_out_rhs_vec, TAU_OUT_SYSTEM_NAME, d_tau_stabilization_eps);
    TAU_out_vec->close();
    ...
    d_X_IB_ghost_vecs[part]->close();
    IBAMR_TIMER_STOP(t_compute_fluid_traction);
...
{
    IBAMR_TIMER_START(t_extrapolate_pressure_for_traction);
    ...
    boost::multi_array<double, 2> x_node, Normal_node;
    std::vector<double> x_qp, x_in_qp, x_out_qp;
    VectorValue<double> Normal_qp;
    VectorValue<double> Smoothed_normal;
    ...
    for (PatchLevel<NDIM>::Iterator p(level); p; p++, ++local_patch_num)
    {
        ...
        if(d_use_smoothed_normal_for_traction)
        {
            ...
            for (unsigned int el_it = 0; el_it < num_active_patch_elems; ++el_it)
            {
                ...
                {
                    copy_dof_ids_to_vector(i, Normal_dof_indices, dof_id_scratch);
                    smoothed_normal_dof_map->constrain_element_vector(Normal_rhs_e[i], dof_id_scratch);// F_dof_map.constrain_element_vector(Normal_rhs_e[i], dof_id_scratch);
            ...
            d_fe_data_managers[part]->computeSmoothedL2Projection(
                *Normal_vec, *Normal_rhs_vec, SMOOTHED_NORMAL_SYSTEM_NAME, d_normal_stabilization_eps);
            Normal_vec->close();

            d_smoothed_normal[part]->close();
        }
        ...
        for (unsigned int e_idx = 0; e_idx < num_active_patch_elems; ++e_idx)
        {
            ...
            for (unsigned int qp = 0; qp < n_qp; ++qp)
            {
                ...
                if(d_use_smoothed_normal_for_traction){
                    interpolate(Smoothed_normal, qp, Normal_node, phi_X);
                }
                if (NDIM == 2)
                {
                    dx_dxi[1] = VectorValue<double>(0.0, 0.0, 1.0);
    ...
    IBAMR_TIMER_STOP(t_extrapolate_pressure_for_traction);
...
void
IIMethod::calculateInterfacialFluidForces(const int p_data_idx, double data_time, const unsigned int part)
{
    if (d_compute_fluid_traction[part] && (!d_use_pressure_jump_conditions || !d_use_velocity_jump_conditions))
    {
...
IIMethod::forwardEulerStep(const double current_time, const double new_time)
{
    ...
    for (unsigned int part = 0; part < d_num_parts; ++part)
    {
        ...
        if(current_time==0 || !d_use_Qi_scheme)
        {
            ierr = VecAXPBYPCZ(d_X_half_vecs[part]->vec(), 0.5, 0.5, 0.0, d_X_current_vecs[part]->vec(), d_X_new_vecs[part]->vec());}
        else{
            ierr =VecWAXPY(d_X_half_vecs[part]->vec(), dt, d_U_current_vecs[part]->vec(), d_half_X_vecs[part]->vec());}
        IBTK_CHKERRQ(ierr);
        if(d_coordinate_stabilization_eps >0){
            NumericVector<double>* d_X_half_vecs_1 = d_X_half_vecs[part];
            std::unique_ptr<NumericVector<double> > d_X_half_vecs_2 = d_X_half_vecs_1->clone();
            NumericVector<double>* d_X_new_vecs_1 = d_X_new_vecs[part];
            std::unique_ptr<NumericVector<double> > d_X_new_vecs_2 = d_X_new_vecs_1->clone();
            d_fe_data_managers[part]->computeSmoothedL2Projection(
                *d_X_half_vecs_1, *d_X_half_vecs_2,COORDS_SYSTEM_NAME, d_coordinate_stabilization_eps);
            d_fe_data_managers[part]->computeSmoothedL2Projection(
                *d_X_new_vecs_1, *d_X_new_vecs_2,COORDS_SYSTEM_NAME, d_coordinate_stabilization_eps);
            d_X_half_vecs_2->close();
            d_X_half_vecs_1->close();
            d_X_new_vecs_1->close();
            d_X_new_vecs_2->close();
        }
        d_X_half_vecs[part]->close();
        d_X_new_vecs[part]->close();
    }
...
IIMethod::trapezoidalStep(const double current_time, const double new_time)
{
    ...
    for (unsigned int part = 0; part < d_num_parts; ++part)
    {
        ...
        ierr = VecAXPBYPCZ(d_X_half_vecs[part]->vec(), 0.5, 0.5, 0.0, d_X_current_vecs[part]->vec(), d_X_new_vecs[part]->vec());
        //ierr =
        //VecWAXPY(d_X_half_vecs[part]->vec(), dt, d_U_new_vecs[part]->vec(), d_X_half_vecs[part]->vec());
        if(d_coordinate_stabilization_eps >0){
            NumericVector<double>* d_X_half_vecs_1 = d_X_half_vecs[part];
            std::unique_ptr<NumericVector<double> > d_X_half_vecs_2 = d_X_half_vecs_1->clone();
            NumericVector<double>* d_X_new_vecs_1 = d_X_new_vecs[part];
...
IIMethod::computeLagrangianForce(const double data_time)
{
    ...
    for (unsigned part = 0; part < d_num_parts; ++part)
    {
       ...
       if (MathUtilities<double>::equalEps(data_time, d_current_time))
        ...
        /////////////////////smoothing normal /////////////
        if(d_use_smoothed_normal || d_use_direct_coupling_smoothed_normal)
        {
            for (auto el_it = el_begin; el_it != el_end; ++el_it)
            {
                auto elem = *el_it;
        ...
        ///////////////////////////////////////////////////
        /////////////////////smoothing normal second round /////////////
        for (int i=0; i<d_num_smooth_cycles;++i){
            if(d_use_smoothed_normal || d_use_direct_coupling_smoothed_normal)
            {
                for (auto el_it = el_begin; el_it != el_end; ++el_it)
                {
                    ...
                    fe_interpolator.interpolate(elem);
                    get_values_for_interpolation(x_node, *X_vec, X_dof_indices);
                    get_values_for_interpolation(Smoothed_Normal_node, *Normal_vec, Normal_dof_indices);
                    const unsigned int n_qpoints = qrule->n_points();
                    const size_t n_basis = phi_X.size();
                    const size_t n_basis2 = phi_jump.size();
                    for (unsigned int qp = 0; qp < n_qpoints; ++qp)
                    {
                        interpolate(x, qp, x_node, phi_X);
                        interpolate(Smoothed_Normal, qp, Smoothed_Normal_node, phi_N);
                        for (unsigned int k = 0; k < NDIM - 1; ++k)
                        {
                            interpolate(dx_dxi[k], qp, x_node, *dphi_dxi_X[k]);
                        }
        ...
        for (auto el_it = el_begin; el_it != el_end; ++el_it)
        {
            ...
            for (unsigned int qp = 0; qp < n_qpoints; ++qp)
            {
                ...
                if(d_use_analytic_normal){
                    n=VectorValue<double>(x(0), x(1), 0.0);
                    n=n.unit();
                    std::cout<<"I am activated analytic normal! ("<<n(0)<<","<<n(1)<<")"<<std::endl;
                }
                ...
                for (unsigned int i = 0; i < NDIM; ++i)
                    for (unsigned int k = 0; k < NDIM; ++k){
                        ...
                        //////test///////
                        // DU[i][k] = x(k);//-(dA / da) * (F(i) - F * n * n(i)) * n(k);
                        // F(k) = x(k);// -(dA / da) * (F(i) - F * n * n(i)) * n(k);
                    }
                      ...
                      const double data_time)
{
    ...
    std::vector<std::vector<libMesh::PetscVector<double>*> > vec_collection_update = {
        d_X_IB_ghost_vecs, d_F_IB_ghost_vecs, d_F_half_vecs//,d_smoothed_normal_ghost,d_smoothed_normal
    };
    if (MathUtilities<double>::equalEps(data_time, d_current_time))
    {
    ...
    for (unsigned int part = 0; part < d_num_parts; ++part)
    {
        ...
        PetscVector<double>* smoothed_normal_vec = d_smoothed_normal[part];
        PetscVector<double>* smoothed_normal_ghost_vec = d_smoothed_normal_ghost[part];
        X_vec->localize(*X_ghost_vec);
        F_vec->localize(*F_ghost_vec);
        smoothed_normal_vec->localize(*smoothed_normal_ghost_vec);
        ...
        if (d_use_pressure_jump_conditions || d_use_velocity_jump_conditions)
        {
            imposeJumpConditions(f_data_idx, *P_jump_ghost_vec, DU_jump_ghost_vec, *X_ghost_vec,*smoothed_normal_ghost_vec,*F_ghost_vec, data_time, part);
        }
        if (d_use_velocity_jump_conditions)
        {
            for (unsigned int d = 0; d < NDIM; ++d) d_DU_jump_IB_ghost_vecs[part][d]->close();
        ...
        d_smoothed_normal_ghost[part]->close();
    ...
    for (unsigned int part = 0; part < d_num_parts; ++part)
    {
        ...
        else
        {
            ...
            std::vector<std::string> vector_system_names{
                ...
                NORMAL_VELOCITY_SYSTEM_NAME, TANGENTIAL_VELOCITY_SYSTEM_NAME, FORCE_SYSTEM_NAME, HALF_X_SYSTEM_NAME, VELOCITY_OLD_SYSTEM_NAME
            };
            std::vector<std::string> vector_variable_prefixes{ "X", "dX", "U", "U_n", "U_t", "F","X_half", "U_old" };
            ...
            const auto smoothed_normal_family =
                d_use_smoothed_normal ? d_smoothed_normal_fe_family : d_fe_family[part];

            auto& system = equation_systems->add_system<System>(SMOOTHED_NORMAL_SYSTEM_NAME);

            for (unsigned int d = 0; d < NDIM; ++d) {
                system.add_variable(
                    "N_" + std::to_string(d), d_smoothed_normal_fe_order, smoothed_normal_family);
            }
...
IIMethod::initializeFEData()
{
    ...
    for (unsigned int part = 0; part < d_num_parts; ++part)
    {
        ...
        auto& half_X_system = equation_systems->get_system<System>(HALF_X_SYSTEM_NAME);
        ...
        half_X_system.assemble_before_solve = false;
        half_X_system.assemble();
...
IIMethod::imposeJumpConditions(const int f_data_idx,
                               ...
                               PetscVector<double>& smoothed_normal_ghost_vec,
                               PetscVector<double>& F_ghost_vec,
                               const double data_time,
                               const unsigned int part)
{
    // Extract the mesh.
    //  std::cout<<"jumpcondition"<<std::endl;
    EquationSystems* equation_systems = d_fe_data_managers[part]->getEquationSystems();
    const MeshBase& mesh = equation_systems->get_mesh();
    const unsigned int dim = mesh.mesh_dimension();
...
//    std::ofstream intersection_output;
//    intersection_output.open("intersection_output "+std::to_string(data_time)+" .csv");
    ...
    boost::multi_array<double, 1> P_jump_node;
    boost::multi_array<double, 2> x_node,smoothed_normal_node,F_node;
    std::array<boost::multi_array<double, 2>, NDIM> DU_jump_node;
    std::array<VectorValue<double>, 2> dx_dxi;
    VectorValue<double> n, jn,jn_diag;
    ...
    for (PatchLevel<NDIM>::Iterator p(level); p; p++, ++local_patch_num)
    {
        ...
        std::array<std::map<hier::Index<NDIM>, std::vector<libMesh::Point>, IndexOrder>, NDIM> intersection_points,
            intersection_ref_coords,intersection_pressure_pos_points,intersection_pressure_neg_points,intersection_pressure_pos_ref_coords,intersection_pressure_neg_ref_coords;
        std::array<std::map<hier::Index<NDIM>, std::vector<VectorValue<double> >, IndexOrder>, NDIM>
            intersection_normals,intersection_pressure_pos_normals,intersection_pressure_neg_normals;

        std::array<std::map<hier::Index<NDIM>, std::vector<libMesh::Point>, IndexOrder>, NDIM> intersection_u_points,intersection_u_points_diag,
            intersection_u_ref_coords,intersection_u_ref_coords_diag;
        std::array<std::map<hier::Index<NDIM>, std::vector<VectorValue<double> >, IndexOrder>, NDIM>
            intersection_u_normals,intersection_u_normals_diag;

        std::array<std::array<std::map<hier::Index<NDIM>, std::vector<libMesh::Point>, IndexOrder>, NDIM>, NDIM>
            intersectionSide_u_points,intersectionSide_u_points_diag, intersectionSide_u_ref_coords,intersectionSide_u_ref_coords_diag;
        std::array<std::array<std::map<hier::Index<NDIM>, std::vector<VectorValue<double> >, IndexOrder>, NDIM>, NDIM>
            intersectionSide_u_normals,intersectionSide_u_normals_diag;

        // Loop over the elements.
        std::vector<libMesh::dof_id_type> dof_id_scratch;
        int axis_intersection_count = 0;
        int diagonal_intersection_count = 0;
        ...
        for (size_t e_idx = 0; e_idx < num_active_patch_elems; ++e_idx)
        {
            ...
            for (unsigned int axis = 0; axis < NDIM; ++axis)
            {
                ...
                // interest.
                VectorValue<double> q, q_diagonal_pos,q_diagonal_neg,q_pressure_diagonal_pos,q_pressure_diagonal_neg;
                q(axis) = 1.0;
                ...
                for (BoxIterator<NDIM> b(axis_box); b; b++)
                {
                    ...
                    libMesh::Point r,r_velocity,r_top,r_bottom;
                    std::array<libMesh::Point, NDIM - 1> rs;

                    for (unsigned int d = 0; d < NDIM; ++d)
                    {
                       // r(d) = (d == axis ? 0.0 : x_lower[d] + dx[d] * (static_cast<double>(i_c(d) - patch_lower[d]) + 0.5));
                       r(d) = x_lower[d] + dx[d] * (static_cast<double>(i_c(d) - patch_lower[d]) + 0.5);// (d == axis ?  0 : x_lower[d] + dx[d] * (static_cast<double>(i_c(d) - patch_lower[d]) + 0.5));
                       r_velocity(d) = (d == axis ?  x_lower[d] + dx[d] * (static_cast<double>(i_c(d) - patch_lower[d])) : x_lower[d] + dx[d] * (static_cast<double>(i_c(d) - patch_lower[d]) + 0.5));
                       r_top(d) = (d == axis ? x_lower[d] + dx[d] * (static_cast<double>(i_c(d) - patch_lower[d]) + 0.5):x_lower[d] + dx[d] * (static_cast<double>(i_c(d) - patch_lower[d]) + 1.5));
                       r_bottom(d) = (d == axis ? x_lower[d] + dx[d] * (static_cast<double>(i_c(d) - patch_lower[d]) + 0.5):x_lower[d] + dx[d] * (static_cast<double>(i_c(d) - patch_lower[d]) -0.5));
                        for (unsigned int l = 0; l < NDIM - 1; ++l)
                        {
                           // rs[l](d) =
                           //     (d == axis ? 0.0 :
                           ...
                           //                  x_lower[d] + dx[d] * (static_cast<double>(i_c(d) - patch_lower[d]) + 0.5));
                            rs[l](d) =  (d == axis ? x_lower[d] + dx[d] * (static_cast<double>(i_c(d) - patch_lower[d])+0.5)  :  d == SideDim[axis][l] ? x_lower[d] + dx[d] * (static_cast<double>(i_c(d) - patch_lower[d])) : x_lower[d] + dx[d] * (static_cast<double>(i_c(d) - patch_lower[d]) + 0.5));
                        }
                    }
                    std::vector<std::pair<double, libMesh::Point> > intersections, intersectionsDiagonal_pos,intersectionsDiagonal_neg, intersectionsDiagonalVelocity_pos,intersectionsDiagonalVelocity_neg;
                    std::array<std::vector<std::pair<double, libMesh::Point> >, NDIM - 1> intersectionsSide,intersectionsSideDiagonal;
                    ...
                    if(d_trial_inconsistent_force){
                        ...
                        const int axis_diagonal= 1-axis;
                        q_diagonal_pos(axis) = dx[axis];
                        q_diagonal_neg(axis) = dx[axis];

                        q_diagonal_pos(axis_diagonal)= axis==0 ? -dx[axis_diagonal]:dx[axis_diagonal];
                        q_diagonal_pos = q_diagonal_pos.unit();
                        q_diagonal_neg(axis_diagonal)= axis==0 ? dx[axis_diagonal]:-dx[axis_diagonal];
                        q_diagonal_neg = q_diagonal_neg.unit();

                        q_pressure_diagonal_pos(axis) = dx[axis];
                        q_pressure_diagonal_neg(axis) = dx[axis];
                        q_pressure_diagonal_pos(axis_diagonal) =dx[axis_diagonal]*2;
                        q_pressure_diagonal_neg(axis_diagonal) =-dx[axis_diagonal]*2;
                        q_pressure_diagonal_pos = q_pressure_diagonal_pos.unit();
                        q_pressure_diagonal_neg = q_pressure_diagonal_neg.unit();


#if (NDIM == 2)


                        intersect_line_with_edge_non_coordinate(
                            intersectionsDiagonal_neg, static_cast<Edge*>(elem), r_top, q_pressure_diagonal_neg, tolerance);
                        intersect_line_with_edge_non_coordinate(
                            intersectionsDiagonal_pos, static_cast<Edge*>(elem), r_bottom, q_pressure_diagonal_pos, tolerance);

                        intersect_line_with_edge_non_coordinate(intersectionsDiagonalVelocity_pos, static_cast<Edge*>(elem), r_velocity,
                                                                q_diagonal_pos, tolerance);
                        ...
                        {
#if (NDIM == 2)
                            intersect_line_with_edge_non_coordinate(intersectionsSideDiagonal[l], static_cast<Edge*>(elem), rs[l],
                                                                    q_diagonal_pos, tolerance);
#endif
                        }
                        if (d_use_pressure_jump_conditions)
                        {
                            for (unsigned int k = 0; k < intersectionsDiagonal_pos.size(); ++k)
                            {
                                const libMesh::Point x = r + intersectionsDiagonal_pos[k].first * q;
                                const libMesh::Point& xi = intersectionsDiagonal_pos[k].second;
...
//                                i_s(axis) = boost::math::iround((x(axis) - x_lower[axis]) / dx[axis]) + patch_lower[axis];
                                if (extended_box.contains(i_s))
                                {
                                    ...
                                    for (int shift = -1; shift <= 1; ++shift)
                                    {
                                        ...
                                        const std::vector<libMesh::Point>& candidate_coords =
                                            intersection_pressure_pos_points[axis][i_s_prime];
                                        const std::vector<libMesh::Point>& candidate_ref_coords =
                                            intersection_pressure_pos_ref_coords[axis][i_s_prime];
                                        const std::vector<VectorValue<double> >& candidate_normals =
                                            intersection_pressure_pos_normals[axis][i_s_prime];
                                    ...
                                    if (!found_same_intersection_point)
                                    {
                                        ...
                                        if (side_ghost_boxes[axis].contains(i_s))
                                        {
                                            ...
                                            {
                                                if(d_F_trial){
                                                    interpolate(&jn(0), 0, F_node, phi_X);
                                                    jn=-jn;
                                                }
                                                else{
                                                    interpolate(&jn(0), 0, DU_jump_node[0], phi_P_jump);
                                                }
                                                // std::cout<<"print out force at pressure"<<jn(0)<<std::endl;
                                                // std::cout<<k<<"intersection :normal vector:"<<n<<std::endl;
                                                // std::cout<<k<<"intersection :direct forces:"<<jn<<std::endl;
                                                double C_p=-jn*n;
                                            }
                                            //                                        std::cout<<" correct normal axis "<<n(axis)<<"directional "<<(n*q_diagonal_pos)<<std::endl;
                                            //                                        std::cout<<"correct coordinate "<<x<<std::endl;
                                            //                                        std::cout<<"correct jump value "<<C_p<<std::endl;
                                            //                                        std::cout<<" corrected i_s "<<i_s<<std::endl;
                                            const double sgn = n(axis) > 0.0 ? 1.0 : n(axis) < 0.0 ? -1.0 : 0.0;
                                            (*f_data)(i_s) += (d_isotropic_force_alpha1/2)*sgn * (C_p / dx[axis]);
                                        }

                                        // Keep track of the positions where we have
                                        // imposed jump conditions.
                                        intersection_pressure_pos_points[axis][i_s].push_back(x);
                                        intersection_pressure_pos_ref_coords[axis][i_s].push_back(xi);
                                        intersection_pressure_pos_normals[axis][i_s].push_back(n);
                                    }
                                }
                            }
                            for (unsigned int k = 0; k < intersectionsDiagonal_neg.size(); ++k)
                            {
                                const libMesh::Point x = r + intersectionsDiagonal_neg[k].first * q;
                                const libMesh::Point& xi = intersectionsDiagonal_neg[k].second;
                                SideIndex<NDIM> i_s(i_c, axis, 0);
                                i_s(axis) = boost::math::iround((x(axis) - x_lower[axis]) / dx[axis]) + patch_lower[axis];
                                if (extended_box.contains(i_s))
                                {
                                    ...
                                    for (int shift = -1; shift <= 1; ++shift)
                                    {
                                        ...
                                        i_s_prime(axis) += shift;
                                        const std::vector<libMesh::Point>& candidate_coords =
                                            intersection_pressure_neg_points[axis][i_s_prime];
                                        const std::vector<libMesh::Point>& candidate_ref_coords =
                                            intersection_pressure_neg_ref_coords[axis][i_s_prime];
                                        const std::vector<VectorValue<double> >& candidate_normals =
                                            intersection_pressure_neg_normals[axis][i_s_prime];
                                    ...
                                    if (!found_same_intersection_point)
                                    {
                                        ...
                                        // Keep track of the positions where we have
                                        // imposed jump conditions.
                                        intersection_pressure_neg_points[axis][i_s].push_back(x);
                                        intersection_pressure_neg_ref_coords[axis][i_s].push_back(xi);
                                        intersection_pressure_neg_normals[axis][i_s].push_back(n);
                                    }
                                }
                            }
                        ...
                        if (d_use_velocity_jump_conditions)
                        {
                            ...
                            for (unsigned int k = 0; k < intersectionsDiagonalVelocity_pos.size(); ++k)
                            {
                                libMesh::Point xu = r_velocity + intersectionsDiagonalVelocity_pos[k].first * q_diagonal_pos;
                                const libMesh::Point& xui = intersectionsDiagonalVelocity_pos[k].second;
                                SideIndex<NDIM> i_s_um(i_c, axis, 0);
                                hier::Index<NDIM> i_c_neighbor = i_c;
                                i_c_neighbor(axis) += 1;
                                i_c_neighbor(axis_diagonal) = axis==0? i_c(axis_diagonal)- 1:i_c(axis_diagonal)+1;
                                ...
                                i_s_um(axis) =
                                    boost::math::iround((xu(axis) - x_lower[axis]) / dx[axis] - 0.5) + patch_lower[axis];
                                if (fmod(xu(axis_diagonal) - x_lower[axis_diagonal], dx[axis_diagonal]) >= 0.5 * dx[axis_diagonal])
                                { i_s_up(axis_diagonal) = axis==0 ?
                                                                boost::math::iround((xu(axis_diagonal) - x_lower[axis_diagonal]) / dx[axis_diagonal] -0.5) + patch_lower[axis_diagonal]:
                                                                boost::math::iround((xu(axis_diagonal) - x_lower[axis_diagonal]) / dx[axis_diagonal] ) + patch_lower[axis_diagonal];
                                    i_s_um(axis_diagonal) = axis == 0 ? boost::math::iround((xu(axis_diagonal) - x_lower[axis_diagonal]) / dx[axis_diagonal]) + patch_lower[axis_diagonal]:
                                                                        boost::math::iround((xu(axis_diagonal) - x_lower[axis_diagonal]) / dx[axis_diagonal] - 0.5) + patch_lower[axis_diagonal];
                                }
                                else if (fmod((xu(axis_diagonal) - x_lower[axis_diagonal]), dx[axis_diagonal]) < 0.5 * dx[axis_diagonal])
                                { i_s_up(axis_diagonal) = axis==0 ?
                                                                boost::math::iround((xu(axis_diagonal) - x_lower[axis_diagonal]) / dx[axis_diagonal] -1) + patch_lower[axis_diagonal]:
                                                                boost::math::iround((xu(axis_diagonal) - x_lower[axis_diagonal]) / dx[axis_diagonal] ) + patch_lower[axis_diagonal];
                                    i_s_um(axis_diagonal) = axis == 0 ? boost::math::iround((xu(axis_diagonal) - x_lower[axis_diagonal]) / dx[axis_diagonal] -0.5) + patch_lower[axis_diagonal]:
                                                                        boost::math::iround((xu(axis_diagonal) - x_lower[axis_diagonal]) / dx[axis_diagonal] -1) + patch_lower[axis_diagonal];
                                }
                                ...
                                if (extended_box.contains(i_s_up) && extended_box.contains(i_s_um))
                                {
                                    ...
                                    for (int shift = -1; shift <= 1; ++shift)
                                    {
                                        ...
                                        i_s_prime(axis) += shift;
                                        const std::vector<libMesh::Point>& candidate_coords =
                                            intersection_u_points_diag[axis][i_s_prime];
                                        const std::vector<libMesh::Point>& candidate_ref_coords =
                                            intersection_u_ref_coords_diag[axis][i_s_prime];
                                        const std::vector<VectorValue<double> >& candidate_normals =
                                            intersection_u_normals_diag[axis][i_s_prime];
                                    ...
                                    if (!found_same_intersection_point)
                                    {
                                        ...
                                        const double x_cell_bdry_um_axis =
                                            x_lower[axis] +
                                            static_cast<double>(i_s_um(axis) - patch_lower[axis]) * dx[axis];
                                        const double x_cell_bdry_um_diag =
                                            x_lower[axis_diagonal] +
                                            static_cast<double>(i_s_um(axis_diagonal) - patch_lower[axis_diagonal]+0.5) * dx[axis_diagonal];
                                        const double x_cell_bdry_up_axis =
                                            x_lower[axis] +
                                            static_cast<double>(i_s_up(axis) - patch_lower[axis]) * dx[axis];
                                        const double x_cell_bdry_up_diag =
                                            x_lower[axis_diagonal] +
                                            static_cast<double>(i_s_up(axis_diagonal) - patch_lower[axis_diagonal]+0.5) * dx[axis_diagonal];
                                        const double sdh_um = sqrt((xu(axis) - x_cell_bdry_um_axis)*(xu(axis) - x_cell_bdry_um_axis)+(xu(axis_diagonal) - x_cell_bdry_um_diag)*(xu(axis_diagonal) - x_cell_bdry_um_diag)); // Signed Distance h
                                        const double sdh_up = -sqrt((xu(axis) - x_cell_bdry_up_axis)*(xu(axis) - x_cell_bdry_up_axis)+(xu(axis_diagonal) - x_cell_bdry_up_diag)*(xu(axis_diagonal) - x_cell_bdry_up_diag)); // Signed Distance h
                                        double h_diagonal = sqrt((dx[axis] * dx[axis])+(dx[axis_diagonal] * dx[axis_diagonal]));
//                                        std::cout<<"Print up 1"<<"( "<<x_cell_bdry_up_axis<<","<<x_cell_bdry_up_diag<<")"<<std::endl;
//                                        std::cout<<"Print um 1"<<"( "<<x_cell_bdry_um_axis<<","<<x_cell_bdry_um_diag<<")"<<std::endl;
//                                        std::cout<<"Print xu 1"<<"( "<<xu(axis)<<","<<xu(axis_diagonal)<<")"<<std::endl;
//                                        std::cout<<"h"<<"( "<<dx[axis]<<")"<<std::endl;
//                                        std::cout<<"ratio"<<"( "<<(xu(axis_diagonal)-x_cell_bdry_um_diag)/(xu(axis)-x_cell_bdry_um_axis)<<","<<(x_cell_bdry_up_diag-x_cell_bdry_um_diag)/(x_cell_bdry_up_axis-x_cell_bdry_um_axis)<<")"<<std::endl;
                                        double ratio1=(xu(axis_diagonal)-x_cell_bdry_um_diag)/(xu(axis)-x_cell_bdry_um_axis);
                                        double ratio2=(x_cell_bdry_up_diag-x_cell_bdry_um_diag)/(x_cell_bdry_up_axis-x_cell_bdry_um_axis);
//                                        std::cout<<"axis"<<"( "<<axis<<","<<axis_diagonal<<")"<<std::endl;
//                                        std::cout<<"sdh_um"<<"( "<<sdh_um<<","<<h_diagonal<<")"<<std::endl;
                                       // TBOX_ASSERT( ratio1==ratio2 );
//                                        if(axis==0){
//                                        intersection_output << xu(0) << " " << xu(1)  << std::endl;
//                                        }

                                        TBOX_ASSERT((sdh_um) < h_diagonal && sdh_um > 0);
                                        TBOX_ASSERT(fabs(sdh_up) < h_diagonal && sdh_up < 0);
                                            ...
                                            side_ghost_boxes[axis].contains(i_s_um))
                                        {
                                            ...
                                            interpolate(&jn_diag(0), 0, DU_jump_node[axis], phi_P_jump);
                                            ...
                                            C_u_up = sdh_up * (jn(axis)*q_diagonal_pos(axis)+jn_diag(axis_diagonal)*q_diagonal_pos(axis_diagonal));
                                            C_u_um = sdh_um * (jn(axis)*q_diagonal_pos(axis)+jn_diag(axis_diagonal)*q_diagonal_pos(axis_diagonal));

                                            //const double sgn = (r-xu)*n > 0.0 ? 1.0 : (r-xu)*n < 0.0 ? -1.0 : 0.0;
                                            const double sgn = (n*q_diagonal_pos) > 0.0 ? 1.0 : (n*q_diagonal_pos) < 0.0 ? -1.0 : 0.0;
                                            //horriblely wrongf
                                            // Note that the corrections are applied to opposite sides
                                            (*f_data)(i_s_up) -= d_isotropic_force_alpha2*sgn* (C_u_um / (h_diagonal*h_diagonal));
                                            (*f_data)(i_s_um) += d_isotropic_force_alpha2*sgn * (C_u_up / (h_diagonal*h_diagonal));
                                        }

                                        // Keep track of the positions where we have
                                        // imposed jump conditions.
                                        intersection_u_points_diag[axis][i_s_um].push_back(xu);
                                        intersection_u_ref_coords_diag[axis][i_s_um].push_back(xui);
                                        intersection_u_normals_diag[axis][i_s_um].push_back(n);
                                    }
                                }
                            }
...
//
                            ...
                            {
                                for (unsigned int k = 0; k < intersectionsSideDiagonal[j].size(); ++k)
                                {
                                    libMesh::Point xu = rs[j] + intersectionsSideDiagonal[j][k].first * q_diagonal_pos;
                                    const libMesh::Point& xui = intersectionsSideDiagonal[j][k].second;
                                    ...
                                    if (xu(axis) - x_lower[axis] > 0.0)
                                    {
                                        if (fmod(xu(axis) - x_lower[axis], dx[axis]) >= 0.5 * dx[axis])
                                        {
                                            ...
                                            i_side_um(axis) =
                                                ...
                                                patch_lower[axis];
                                            i_side_up(axis_diagonal) = axis == 0? boost::math::iround((xu(axis_diagonal) - x_lower[axis_diagonal]) / dx[axis_diagonal] - 0.5) + patch_lower[axis_diagonal]:
                                                                                   boost::math::iround((xu(axis_diagonal) - x_lower[axis_diagonal]) / dx[axis_diagonal] + 0.5) + patch_lower[axis_diagonal];
                                            i_side_um(axis_diagonal) = axis == 0? boost::math::iround((xu(axis_diagonal) - x_lower[axis_diagonal]) / dx[axis_diagonal] + 0.5) + patch_lower[axis_diagonal]:
                                                                                   boost::math::iround((xu(axis_diagonal) - x_lower[axis_diagonal]) / dx[axis_diagonal] - 0.5) + patch_lower[axis_diagonal];
                                            i_s_up = i_side_up;
                                            i_s_um = i_side_um;
                                        }
                                    ...
                                    if (extended_side_box.contains(i_s_up) && extended_side_box.contains(i_s_um))
                                    {
                                        ...
                                        for (int shift = -1; shift <= 1; ++shift)
                                        {
                                            ...
                                            const std::vector<libMesh::Point>& candidate_coords =
                                                intersectionSide_u_points_diag[j][axis][i_s_prime];
                                            const std::vector<libMesh::Point>& candidate_ref_coords =
                                                intersectionSide_u_ref_coords_diag[j][axis][i_s_prime];
                                            const std::vector<VectorValue<double> >& candidate_normals =
                                                intersectionSide_u_normals_diag[j][axis][i_s_prime];
                                        ...
                                        if (!found_same_intersection_point)
                                        {
                                            ...
                                            const double x_mid_side_up_axis =
                                                x_lower[axis] +
                                                static_cast<double>(i_s_up(axis) - patch_lower[axis] + 0.5) * dx[axis];
                                            const double x_mid_side_up_diagonal =
                                                x_lower[axis_diagonal] +
                                                static_cast<double>(i_s_up(axis_diagonal) - patch_lower[axis_diagonal]) * dx[axis_diagonal];
                                            const double x_mid_side_um_axis =
                                                x_lower[axis] +
                                                static_cast<double>(i_s_um(axis) - patch_lower[axis] + 0.5) * dx[axis];
                                            const double x_mid_side_um_diagonal =
                                                x_lower[axis_diagonal] +
                                                static_cast<double>(i_s_um(axis_diagonal) - patch_lower[axis_diagonal]) * dx[axis_diagonal];

                                            TBOX_ASSERT(xu(axis) <= x_mid_side_up_axis);
                                            TBOX_ASSERT(xu(axis) > x_mid_side_um_axis);

                                            //const double sdh_up = xu(axis) - x_mid_side_up; // Signed Distance h
                                            //const double sdh_um = xu(axis) - x_mid_side_um;
                                            const double sdh_up = -sqrt((xu(axis) - x_mid_side_up_axis)*(xu(axis) - x_mid_side_up_axis)+(xu(axis_diagonal) - x_mid_side_up_diagonal)*(xu(axis_diagonal) - x_mid_side_up_diagonal)); // Signed Distance h
                                            const double sdh_um = sqrt((xu(axis) - x_mid_side_um_axis)*(xu(axis) - x_mid_side_um_axis)+(xu(axis_diagonal) - x_mid_side_um_diagonal)*(xu(axis_diagonal) - x_mid_side_um_diagonal)); // Signed Distance h
                                            double h_diagonal = sqrt((dx[axis] * dx[axis])+(dx[axis_diagonal] * dx[axis_diagonal]));
//                                            std::cout<<"Print up"<<"( "<<x_mid_side_up_axis<<","<<x_mid_side_up_diagonal<<")"<<std::endl;
//                                            std::cout<<"Print um"<<"( "<<x_mid_side_um_axis<<","<<x_mid_side_um_diagonal<<")"<<std::endl;
//                                            std::cout<<"Print xu"<<"( "<<xu(axis)<<","<<xu(axis_diagonal)<<")"<<std::endl;
//                                            std::cout<<"h"<<"( "<<dx[axis]<<")"<<std::endl;
//                                            std::cout<<"ratio2"<<"( "<<(xu(axis_diagonal)-x_mid_side_um_diagonal)/(xu(axis)-x_mid_side_um_axis)<<","<<(x_mid_side_up_diagonal-x_mid_side_um_diagonal)/(x_mid_side_up_axis-x_mid_side_um_axis)<<")"<<std::endl;

                                            double ratio1=(xu(axis_diagonal)-x_mid_side_um_diagonal)/(xu(axis)-x_mid_side_um_axis);
                                            double ratio2=(x_mid_side_up_diagonal-x_mid_side_um_diagonal)/(x_mid_side_up_axis-x_mid_side_um_axis);
//                                            std::cout<<"axis"<<"( "<<axis<<","<<axis_diagonal<<")"<<std::endl;
                                            //                                        std::cout<<"sdh_um"<<"( "<<sdh_um<<","<<h_diagonal<<")"<<std::endl;
                                           // TBOX_ASSERT( ratio1==ratio2 );
                                          //  std::cout<<"sdh_um"<<"( "<<sdh_um<<","<<h_diagonal<<")"<<std::endl;
                                            TBOX_ASSERT(abs(abs(xu(axis)-x_mid_side_um_axis)-abs(xu(axis_diagonal)-x_mid_side_um_diagonal) )<  0.0000000001  );
                                                ...
                                                side_ghost_boxes[SideDim[axis][j]].contains(i_s_um))
                                            {
                                                ...
                                                interpolate(&jn_diag(0), 0, DU_jump_node[SideDim[axis][j]], phi_P_jump);
                                                ...
                                                if (d_use_direct_coupling)
                                                {
                                                    ...
                                                    jn(axis)=(jn(SideDim[axis][j])-jn*n*n(SideDim[axis][j]))*n(axis);
                                                }
                                                C_u_um = sdh_um * (jn(axis)*q_diagonal_pos(axis)+jn_diag(axis_diagonal)*q_diagonal_pos(axis_diagonal));
                                                C_u_up = sdh_up * (jn(axis)*q_diagonal_pos(axis)+jn_diag(axis_diagonal)*q_diagonal_pos(axis_diagonal));
                                                ...
                                                (*f_data)(i_s_um) += d_isotropic_force_alpha3*sgn * (C_u_up / (h_diagonal*h_diagonal));
                                                (*f_data)(i_s_up) -= d_isotropic_force_alpha3*sgn * (C_u_um / (h_diagonal*h_diagonal));
                                            }
                                            intersectionSide_u_points_diag[j][axis][i_s_um].push_back(xu);
                                            intersectionSide_u_ref_coords_diag[j][axis][i_s_um].push_back(xui);
                                            intersectionSide_u_normals_diag[j][axis][i_s_um].push_back(n);
                                        }
                                    }
                                }
                            }
                    ...
                    if (d_use_pressure_jump_conditions)
                    {
                        ...
                        for (unsigned int k = 0; k < intersections.size(); ++k)
                        {
                            ...
                            SideIndex<NDIM> i_s_up;
                            i_s_up = i_s;
                            i_s_up(axis_diagonal)+=1;
                            SideIndex<NDIM> i_s_um;
                            i_s_um = i_s;
                            i_s_um(axis_diagonal)-=1;
...
//                                        std::cout<<" corrected i_s "<<i_s<<std::endl;
                                        const double sgn = n(axis) > 0.0 ? 1.0 : n(axis) < 0.0 ? -1.0 : 0.0;
                                        (*f_data)(i_s) += (1-d_isotropic_force_alpha1)*sgn * (C_p / dx[axis]);
                    ...
                    if (d_use_velocity_jump_conditions)
                    {
                        for (unsigned int k = 0; k < intersections.size(); ++k)
                        {
                            ...
                            hier::Index<NDIM> i_c_neighbor_upper_up = i_c;
                            hier::Index<NDIM> i_c_neighbor_upper_um = i_c;
                            i_c_neighbor_upper_up(1-axis)+=1;
                            i_c_neighbor_upper_um(1-axis)+=1;

                            SideIndex<NDIM> i_s_up_upper_up(i_c_neighbor_upper_up, axis, 0);
                            SideIndex<NDIM> i_s_up_upper_um(i_c_neighbor_upper_um, axis, 0);
                            hier::Index<NDIM> i_c_neighbor_lower_up = i_c;
                            hier::Index<NDIM> i_c_neighbor_lower_um = i_c;
                            i_c_neighbor_lower_up(1-axis)-=1;
                            i_c_neighbor_lower_um(1-axis)-=1;
                            SideIndex<NDIM> i_s_up_lower_up(i_c_neighbor_lower_up, axis, 0);
                            SideIndex<NDIM> i_s_up_lower_um(i_c_neighbor_lower_um, axis, 0);
                            if(d_trial){
                                i_s_up_upper_up(axis) =
                                    boost::math::iround((xu(axis) - x_lower[axis]) / dx[axis] + 0.5) + patch_lower[axis];
                                i_s_up_upper_um(axis) =
                                    boost::math::iround((xu(axis) - x_lower[axis]) / dx[axis] - 0.5) + patch_lower[axis];

                                i_s_up_lower_up(axis) =
                                    boost::math::iround((xu(axis) - x_lower[axis]) / dx[axis] + 0.5) + patch_lower[axis];
                                i_s_up_lower_um(axis) =
                                    boost::math::iround((xu(axis) - x_lower[axis]) / dx[axis] - 0.5) + patch_lower[axis];
                            ...
                            if (extended_box.contains(i_s_up) && extended_box.contains(i_s_um))
                            {
                                ...
                                if (!found_same_intersection_point)
                                {
                                        ...
                                        side_ghost_boxes[axis].contains(i_s_um))
                                    {
                                        ...
                                        const double sgn = n(axis) > 0.0 ? 1.0 : n(axis) < 0.0 ? -1.0 : 0.0;
                                        // Note that the corrections are applied to opposite sides
                                        (*f_data)(i_s_up) -=  (1-d_isotropic_force_alpha2)*sgn * (C_u_um / (dx[axis] * dx[axis]));
                                        (*f_data)(i_s_um) +=  (1-d_isotropic_force_alpha2)*sgn * (C_u_up / (dx[axis] * dx[axis]));
                        ...
                        for (unsigned int j = 0; j < NDIM - 1; ++j)
                        {
                            for (unsigned int k = 0; k < intersectionsSide[j].size(); ++k)
                            {
                                ...
                                SideIndex<NDIM> i_s_up;
                                SideIndex<NDIM> i_s_um;
                                SideIndex<NDIM> i_s_up_upper_up;
                                SideIndex<NDIM> i_s_up_upper_um;
                                SideIndex<NDIM> i_s_up_lower_up;
                                SideIndex<NDIM> i_s_up_lower_um;
                                if (xu(axis) - x_lower[axis] > 0.0)
                                {
                                    if (fmod(xu(axis) - x_lower[axis], dx[axis]) >= 0.5 * dx[axis])
                                    {
                                ...
                                if(d_trial){
                                    i_s_up_upper_up= i_s_up;
                                    i_s_up_upper_um= i_s_um;
                                    i_s_up_upper_up(1-axis)+=1;
                                    i_s_up_upper_um(1-axis)+=1;

                                    i_s_up_lower_up= i_s_up;
                                    i_s_up_lower_um= i_s_um;
                                    i_s_up_lower_up(1-axis)-=1;
                                    i_s_up_lower_um(1-axis)-=1;
                                }
                                ...
                                if (extended_side_box.contains(i_s_up) && extended_side_box.contains(i_s_um))
                                {
                                    ...
                                    if (!found_same_intersection_point)
                                    {
                                            ...
                                            side_ghost_boxes[SideDim[axis][j]].contains(i_s_um))
                                        {
                                            ...
                                            (*f_data)(i_s_um) += (1-d_isotropic_force_alpha3)*sgn * (C_u_up / (dx[axis] * dx[axis]));
                                            (*f_data)(i_s_up) -= (1-d_isotropic_force_alpha3)*sgn * (C_u_um / (dx[axis] * dx[axis]));
...
//        std::cout<<"diagonal intersection no"<< diagonal_intersection_count<<"axis intersection no"<< axis_intersection_count<<std::endl;
    }
//    intersection_output.close();
    return;
                            ...
                            unsigned int restart_restore_number)
{
    ...
    d_compute_fluid_traction.resize(d_num_parts);
    ...
    IBAMR_DO_ONCE(t_preprocess_integrate_data = set_timer("IBAMR::IIMMethod::preprocessIntegrateData()");
                  ...
                  t_compute_fluid_traction = set_timer("IBAMR::IIMMethod::computeFluidTraction()");
                  t_extrapolate_pressure_for_traction = set_timer("IBAMR::IIMMethod::extrapolate_pressure_for_traction()");
...
//if (db->isBool("compute_fluid_traction")) d_compute_fluid_traction = db->getBool("compute_fluid_traction");
    ...
    if (db->isDouble("wss_stabilization_eps")) d_wss_stabilization_eps = db->getDouble("wss_stabilization_eps");
    if (db->isDouble("tau_stabilization_eps")) d_tau_stabilization_eps = db->getDouble("tau_stabilization_eps");
    ...
    if (db->isBool("use_smoothed_normal"))
    {
        ...
        if (db->isBool("smoothed_normal_fe_order")) d_smoothed_normal_fe_order= Utility::string_to_enum<Order>(db->getString("smoothed_normal_fe_order"));
    ...
    if (db->isBool("trial"))
    {
        d_trial= db->getBool("trial");
    }
    if (db->isBool("use_stablized_tangent_velocity_update_structure"))
    {
        d_use_stablized_tangent_velocity_update_structure = db->getBool("use_stablized_tangent_velocity_update_structure");
    }
    ...
    //    }
    if (db->isDouble("num_smooth_cycles"))
    {
        d_num_smooth_cycles = db->getDouble("num_smooth_cycles");
    }
    if (db->isDouble("isotropic_force_alpha1"))
    {
        d_isotropic_force_alpha1 = db->getDouble("isotropic_force_alpha1");
    }
    if (db->isDouble("isotropic_force_alpha2"))
    {
        d_isotropic_force_alpha2 = db->getDouble("isotropic_force_alpha2");
    }
    if (db->isDouble("isotropic_force_alpha3"))
    {
        d_isotropic_force_alpha3 = db->getDouble("isotropic_force_alpha3");
    }
    if (db->isBool("trial_inconsistent_force"))
    {
        d_trial_inconsistent_force= db->getBool("trial_inconsistent_force");
    }
    if (db->isBool("use_incorrected_velocity_interpoaltion_for_force_predictor"))
    {
        d_use_incorrected_velocity_interpoaltion_for_force_predictor= db->getBool("use_incorrected_velocity_interpoaltion_for_force_predictor");
