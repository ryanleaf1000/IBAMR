...
main(int argc, char* argv[])
{
    ...
    { // cleanup dynamically allocated objects prior to shutdown
        ...
        {
            lower_mesh.add_point(
                libMesh::Point(L * static_cast<Real>(i) / static_cast<Real>(nn),
                               y_position_low + tan(theta) * L * static_cast<Real>(i) / static_cast<Real>(nn),
                               0.0),
                node_id++);
        }
        ...
        for (unsigned int i = 0; i <= nn; i++)
        {
            upper_mesh.add_point(
                libMesh::Point(L * static_cast<Real>(i) / static_cast<Real>(nn),
                               y_position_up + tan(theta) * L * static_cast<Real>(i) / static_cast<Real>(nn),
                               0.0),
                node_id++);
        }
        ...
        ib_method_ops->registerTractionCalc(0);
        ...
        for (PatchLevel<NDIM>::Iterator p(level); p; p++)
        {
            ...
            for (BoxList<NDIM>::Iterator lit(iterate_box_list); lit; lit++)
            {
                ...
                for (Box<NDIM>::Iterator bit(iterate_box); bit; bit++)
                {
                    ...
                    {
                        u_ex = cos(theta) * (p_e * D / (MU * (L / cos(theta) + D * tan(theta)))) *
                               (-xu * sin(theta) + (yu - X1_lower) * cos(theta)) *
                               (1 - (-xu * sin(theta) + (yu - X1_lower) * cos(theta)) / D);
                    }
                    ...
                    {
                        v_ex = sin(theta) * (p_e * D / (MU * (L / cos(theta) + D * tan(theta)))) *
                               (-xv * sin(theta) + (yv - X1_lower) * cos(theta)) *
                               (1 - (-xv * sin(theta) + (yv - X1_lower) * cos(theta)) / D);
                    }
        ...
        for (PatchLevel<NDIM>::Iterator p(level); p; p++)
        {
            ...
            for (BoxList<NDIM>::Iterator lit(iterate_box_list); lit; lit++)
            {
                ...
                for (Box<NDIM>::Iterator bit(iterate_box); bit; bit++)
                {
                    ...
                    double p_ex_qp;
                    if (y < (X1_lower + x * tan(theta) - fac * patch_dx[1]) ||
                        y > (X1_upper + x * tan(theta) + fac * patch_dx[1]))
                    {
                        p_ex_qp = 0.0;
                    }
                    else if (std::abs(y - (X1_lower + x * tan(theta))) <= fac * patch_dx[1] ||
                             std::abs(y - (X1_upper + x * tan(theta))) <= fac * patch_dx[1])
                    {
                        p_ex_qp = p1;
                    }
                    else
                    {
                        p_ex_qp = (-2. * p_e * (x / cos(theta) + (y - X1_lower - x * tan(theta)) * sin(theta)) /
                                       (L / cos(theta) + D * tan(theta)) +
                                   p_e);
                    }
...

    {
        ...
        for (auto el_it = el_begin; el_it != el_end; ++el_it)
        {
            ...
            for (int qp = 0; qp < n_qp; ++qp)
            {
                ...
                double p_ex_qp =
                    (-2.0 * p_e * (X_qp(0) / cos(theta) + (X_qp(1) - X1_lower - X_qp(0) * tan(theta)) * sin(theta)) /
                         (L / cos(theta) + D * tan(theta)) +
                     p_e);
