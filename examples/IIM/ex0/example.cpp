...
main(int argc, char* argv[])
{
    ...
    { // cleanup dynamically allocated objects prior to shutdown
        ...
        if (NDIM == 2 && (elem_type == "TRI3" || elem_type == "TRI6"))
        {
            ...
            if(non_uniform_index!=0){
            for (int k = 0; k < num_circum_nodes; ++k)
            {
                int remainder=k%non_uniform_index;
                std::cout<<"I am free of problems"<<std::endl;
                const double theta = 2.0 * M_PI * ((static_cast<double>(k) / static_cast<double>(num_circum_nodes)) + (interface_mesh_nouniform_bound / static_cast<double>(num_circum_nodes)) * (remainder / (non_uniform_index / 2) - 1));
                std::cout<<"I am free of problems 2"<<std::endl;
                solid_mesh.add_point(libMesh::Point(R * cos(theta), R * sin(theta)));}
                std::cout<<"I am free of problems 5"<<std::endl;
            }
            else{
                for (int k = 0; k < num_circum_nodes; ++k)
